...
const Services = () => {
    ...
    return (
        <section id="services" className="pt-[2rem] px-[2rem] pb-[2rem] sm:px-[7rem]">
            <h2 className="text-[3rem] leading-[3rem] font-[600] text-[#1B82E2] text-center">
                Services
            </h2>
            <div className="mt-[3rem] sm:hidden  max-w-[1900px] mx-auto">
                <h3 className="text-[2rem] font-[600]"><span className="uppercase text-[#1B82E2]">kyc - </span>  Credit and Background Checks</h3>
            </div>
            ...
            <div className="pt-[3.5rem] max-w-[1900px] mx-auto pb-[5rem] sm:pb-[7rem] sm:rounded-[4rem] rounded-[2rem] mt-[5rem] sm:px-[4rem] bg-[#F5F5F5]">
                <h3 className="text-center text-[2rem] sm:text-[3rem]">Direct <span className="text-[#1B82E2] font-[600]">Debit</span> Services</h3>

                <p className="sm:mt-[2.5rem] mt-[1rem] sm:w-[80%] w-[92%] text-[1.6rem] sm:text-[1.8rem] text-center mx-auto text-[#737373]">The methods available for loan collections are just as diverse as the borrowers themselves. One method that stands out for its reliability and directness is Direct Debit. This payment method, accessible to lenders, facilitates the retrieval of funds from a customer's account precisely when their loan payment is due.</p>

                <p className="w-[80%] text-[1.6rem] sm:text-[1.8rem] text-center mx-auto text-[#737373]">
                    with direct debit as a payment method, sellers gain a predictable and efficient way to collect payments directly from customers’ accounts.
                </p>

                <div className="mt-[9rem]">
                    <p className="text-center text-[1.8rem] sm:text-[2rem] font-[500] text-[#737373] w-[80%] mx-auto">Why you should use Direct Debit Collection Method</p>

                    <div className="grid mt-[3rem] px-[1rem] grid-cols-2 sm:grid-cols-4 sm:gap-[2rem] gap-[1.2rem]">
                        {
                            methods.map((method, index) => <Methods
                                key={index}
                                {...method}
                            />)
                        }
...
const Card = ({ title, description, icon }: CardProps) => {
    return (
        <div className="bg-[#F5F5F5] rounded-[2rem] pt-[4rem] pb-[5rem] px-[2.5rem]">
            ...
            <p className="text-[1.8rem] text-[#595959]">{description}</p>
        </div>
    )
}
...
const Methods = ({ title, icon }: MethodsProps) => {
    return (
        <div className="bg-[#FFFFFF] rounded-[1.5rem] py-[3rem]">
            ...
            />
            <p className="sm:max-w-[18rem] max-w-[16rem] mx-auto sm:text-[1.8rem] text-[1.4rem] text-center font-[500]">{title}</p>
        </div>
    )
}
