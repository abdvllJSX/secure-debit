...
const Services = () => {
    ...
    return (
        <section id="services" className="sm:pt-[2rem] px-[2rem] pb-[2rem] sm:px-[7rem]">
            ...
            <div className="pt-[3.5rem] max-w-[1900px] mx-auto pb-[5rem] sm:pb-[7rem] sm:rounded-[4rem] rounded-[2rem] mt-[5rem] sm:px-[4rem] bg-[#F5F5F5]">
                ...
                <p className="sm:mt-[2.5rem] mt-[1rem] sm:w-[80%] w-[92%] text-[1.7rem] sm:text-[1.8rem] text-center mx-auto text-[#737373]">The methods available for loan collections are just as diverse as the borrowers themselves. One method that stands out for its reliability and directness is Direct Debit. This payment method, accessible to lenders, facilitates the retrieval of funds from a customer's account precisely when their loan payment is due.</p>
...
const Methods = ({ title, icon }: MethodsProps) => {
    return (
        <div className="bg-[#FFFFFF] rounded-[1.5rem] py-[3rem]">
            ...
            />
            <p className="sm:max-w-[18rem] sm:w-[100%] w-[87%] mx-auto sm:text-[1.8rem] text-[1.4rem] text-center font-[500]">{title}</p>
        </div>
    )
}
